...
export function AuthModal({ onClose }: { onClose: () => void }) {
  ...
  const handleGmailLogin = async () => {
    ...
    const newUser: User = {
      ...
      handle: fakeEmail.split('@')[0] + Math.floor(Math.random() * 1000),
      avatarUrl: `https://api.dicebear.com/7.x/avataaars/svg?seed=${fakeEmail}`,
      bio: 'BETA user :D',
      following: [],
      followers: [],
      isPrivate: false,
      ...
      <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="p-6">
          ...
          <button 
            ...
            className="mt-6 w-full flex items-center justify-center gap-3 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-200 font-medium py-3 rounded-lg transition-colors"
          >
        ...
        <div className="bg-zinc-50 dark:bg-zinc-950 p-4 text-center">
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            ...
            <button onClick={() => setIsSignUp(!isSignUp)} className="text-pink-600 font-semibold hover:underline">
        ...
        <button onClick={onClose} className="absolute top-4 right-4 text-zinc-500 hover:text-zinc-800 dark:hover:text-white">
